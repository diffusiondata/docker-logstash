...
RUN wget https://download.elastic.co/logstash/logstash/logstash-1.5.1.tar.gz -O /tmp/logstash.tar.gz 2> /dev/null

# Unzip logstash and put in place
RUN tar zxf /tmp/logstash.tar.gz -C /opt && mv /opt/logstash-1.5.1 /opt/logstash && rm -rf /tmp/logstash.tar.gz
